...
class NaiveProgramInterpreter : Interpreter<MachineState, Program, List<Int>> {
    ...
    override fun join(s: MachineState, p: Program): MachineState {
        println("Interpreter functions: ${p.functionDeclarations.size} / Main: ${p.mainFunction.body.exhaustive}")
        
        return interpreter.join(
            s.copy(functionReferences = p.functionDeclarations.associate { it to it.body }),
            p.mainFunction.body
        )
    }
}